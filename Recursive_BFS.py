...
dict_hn={
   ...
   'Neamt':234,'Oradea':380,'Pitesti':100,'Rimnicu':193,'Sibiu':253,'Timisoara':329,
...
dict_gn=dict(
        ...
        Eforie=dict(Hirosova=86),
        Fagaras=dict(Sibiu=99,Bucharest=211),
        Giurgiu=dict(Bucharest=90),
        Hirosova=dict(Urziceni=98,Eforie=86),
        ...
        Sibiu=dict(Arad=140,Oradea=151,Rimnicu=80,Fagaras=99),
...
def get_fn(citystr):
    ...
    while ctr!=len(cities)-1:
        ...
        print('f(n) ',cities[len(cities)-1],' : ',(hn+gn))
    return (hn+gn)
...
def expand(mycities,cityq,goal):
    ...
    if city2expand == goal:
        ans='The Recurssive best path is '+citystr+' with the value as '+str(tot)
